function Weights = FnEstCoeffs(data)
global mdata Ts mu PolyDegree
%function Weights = FnEstCoeffs(data,mdata,h,mu,PolyDegree)
% 
% data = contains the data from the system to identify (W)
...
disp("Length of data: "+string(length(data)));
total_time = tic;
...
%% Need Gram Matrix
    ...
    for i = 1:TotalCenters
        ...
        for j = 1:BasisSize
            ...
            BigMatrix(i,j) = Ts/3*(-2/mu)*diag((W - Centers(:,i))'*(HoldExp.*squeeze(EvalForIntegration(j,:,:))))'*(SimpsonsRuleVector');
        end
    end
